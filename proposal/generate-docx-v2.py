"""Generate proposal v2 DOCX matching the RBC Borealis sample PDF style."""
...
OUTPUT_PATH = os.path.join(OUTPUT_DIR, "proposal-v2.docx")
...
def build_document():
    ...
    # ═══════════════════════════════════════════════
    # SECTION 2: Why it matters (MERGED V1+V2)
    # ═══════════════════════════════════════════════
    add_section_heading(doc, "Why does this matter to our team?")

    add_body(
        doc,
        "We use AI every single day. ChatGPT to debug code at midnight before a deadline. "
        "GitHub Copilot to get unstuck. Cloud platforms to run experiments that our laptops "
        "can\u2019t handle. These tools feel effortless \u2014 a few keystrokes and the answers "
        "appear. But somewhere in the past year, studying systems and sustainability, we "
        "started asking ourselves: where does all of that come from?",
    )

    add_body(
        doc,
        "What we found surprised us. Quebec runs on "
        "hydropower, so a data center there sounds clean. But when we read the Dandres et al. "
        "research, we found that the marginal electricity actually powering new Canadian data "
        "centers \u2014 the electricity that gets added to the grid because the data center "
        "showed up \u2014 has a carbon intensity nearly six times the Canadian average. Why? "
        "Because Canada reduces its hydro exports to the US, and the US compensates by firing "
        "up coal and gas plants. The carbon is real. It\u2019s just invisible under the "
        "accounting methods everyone uses.",
    )

    add_body(
        doc,
        "That\u2019s the kind of problem we want to work on. Siting decisions lock in 20 to "
        "30 years of environmental consequences. Ontario\u2019s Bill 40 now requires ministerial "
        "sign-off before a data center can connect to the grid \u2014 and there\u2019s still no "
        "public tool to help those ministers make a well-informed decision. Our team brings "
        "together backgrounds in computer science, economics, and finance \u2014 a combination "
        "well suited to a problem that is as much about decision-making and policy as it is "
        "about modelling. We want to build something that matters beyond a grade, and we think "
        "an open tool that helps inform this process is exactly the kind of impact we came here "
        "hoping to have.",
    )
