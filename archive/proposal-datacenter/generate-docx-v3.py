"""Generate proposal v3 DOCX matching the RBC Borealis sample PDF style.

v3 change: Replaced the 3-paragraph "Why does this matter to our team?" section
with the tighter single-paragraph version that weaves in specific team experiences.
"""
...
OUTPUT_PATH = os.path.join(OUTPUT_DIR, "proposal-v3.docx")
...
def build_document():
    ...
    # ═══════════════════════════════════════════════
    # SECTION 2: Why it matters — v3 (single paragraph, image version)
    # ═══════════════════════════════════════════════
    add_section_heading(doc, "Why does this matter to our team?")

    add_body(
        doc,
        "We rely on AI every day: ChatGPT to debug code at midnight, Copilot to scaffold "
        "assignments, and cloud GPUs to run models for our research and internships when "
        "our laptops max out. Over the past year, coursework in systems and sustainability, "
        "an internship touching grid reliability, and student research on ML models pushed "
        "us to ask a question we had mostly ignored: what does all this actually cost the "
        "grid and the climate? Digging into Dandres et al. and Ontario\u2019s new Bill 40, "
        "we realized that \u201cclean\u201d Canadian data centers can hide high marginal and "
        "cross-border emissions and that ministers are now expected to approve or deny grid "
        "connections without an open, quantitative tool to see those tradeoffs. As students "
        "spanning computer science, economics, and finance, we feel a responsibility to "
        "build that missing piece: an open Societal Impact Score that turns our own "
        "dependence on AI infrastructure into something that helps communities and "
        "policymakers make better long-term decisions.",
    )
